...
using ApiDotnet.Models.Entites;
...
public class PredictionsController : ControllerBase
{
    ...
    [HttpGet("GetFixtureById/{id}")]
    public async Task<IActionResult> GetFixtureById(int id)
    {
        var doc = await MakeRequest($"https://v3.football.api-sports.io/fixtures?id={id}");
        if (doc == null) return StatusCode(500, "API error");
    
        var responseArray = doc.RootElement.GetProperty("response");
    
        var result = JsonSerializer.Deserialize<List<Fixture>>(responseArray.GetRawText());
